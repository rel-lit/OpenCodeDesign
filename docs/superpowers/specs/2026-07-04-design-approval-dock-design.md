...
2. **只有二进制选择**：`question.ask` 的选项一般是固定 label（如 Apply / Reject），用户无法方便地输入修改意见。Revise 需要借助 custom answer，但 UI 上不够直观。
3. **视觉语义不清晰**：用户难以一眼看出这是一个"设计图变更审批"，而不是普通的问题调查。

因此需要新增一个专门的 **Design Approval Dock**，在复用现有 `DockPrompt` 容器的基础上，为设计审批提供定制化的展示和交互。
...
Design Approval Dock 仍然基于 `question.ask` 事件和 `Question.Service` 的 `reply/reject` API。区别在于：
...
使用 `DockPrompt` 容器，`kind="design-approval"`：
...
- 如果内容过长，区域内部可滚动（参考 `SessionQuestionDock` 的 `overflow-auto` + 最大高度计算）。
...
- 支持多行，自动增高（参考 `SessionQuestionDock` 的 `resizeInput` 实现）。
...
- `packages/app/src/pages/session/composer/session-composer-region.tsx`
  - 在 `SessionQuestionDock` 之前（或替代）渲染 `SessionDesignApprovalDock`。
...
- `packages/session-ui/src/components/dock-prompt.tsx`：只新增 `kind="design-approval"`，容器本身无需修改。
